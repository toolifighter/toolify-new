...
export default class Bewegungsmesser extends React.Component {
	...
	helper = {
		c: 0, x: 0, y: 0, averageIndicator: 0, teilfehler: "false",
	};
	...
	Start = () => {
		if (this.state.buttontitle == "Start") {
			this.setState({
				buttontitle: "Stop", fehlercode1: '', fehlerindicator: false,
			});
			this.durchschnittler = [], this.helper.teilfehler = false, Accelerometer.setUpdateInterval(1);
			Accelerometer.addListener(accelerometerData => {
				this.measurementData.push({
					x: accelerometerData.x * 9.81, y: accelerometerData.y * 9.81,
				})
			});
			...
			if (this.helper.teilfehler == true) {
				this.setState({
					fehlercode1: 'Fehlermessung', fehlerindicator: true, buttontitle: "Start",
				})
	...
	Loschen = () => {
		if (this.state.buttontitle == "Start") {
			...
			this.helper.c = 0;
			this.helper.x = 0, this.helper.y = 0, this.helper.teilfehler = false, this.helper.averageIndicator = 0;
			this.measurementData = [];
		...
		<ScrollView>
			<View style={styles.container}>
				...
				<View style={[styles.inputContainer, {maxHeight: 350}]}>
					<Text style={styles.inputText}>Legen Sie Ihr Smartphone auf eine gerade Unterlage. Starten Sie
						die Messung und bewgen Sie das Gerät zügig zur Zielposition. Stoppen Sie die Messung. Führen
						sie diese Messung erneut in entgegengesetzte Richtung durch, um Ungenauigkeiten zu
						vermeiden. Sie können auch weitere Messungen durchführen.</Text>
				</View>
